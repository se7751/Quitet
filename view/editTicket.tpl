...
<html>
	...
	<body></br>
		<h1>チケット編集</h1>
		<form action="excEditTicket.php" method="post" name="newProject" onSubmit="return check()" enctype="multipart/form-data">
		<input type="hidden" name="choose"/>
			<!-- コメントテーブルに格納する投稿ID -->
			<input type="hidden" name="id" value="{$ticket_id}" />
			<table id="test">{foreach from=$ticket item=tic}
					<tr>
						<th>タイトル</th>
						<td><input type="text" name="title" id="title" value="{$tic.title}" /></td>
					</tr>
					<tr>
						<th>ステータス</th>
						<td>
					...
					<tr>
						<th>期日</th>
						<td>
							<input type="text" name="kijitu" id="date" size="10" value="{$tic.kijitu}" />
					...
					<tr>
						<th>詳細</th>
						<td><textarea name="body" id="body" cols="40" rows="8">{$tic.body}</textarea></td>
					</tr>
					...
					<tr>
                        ...
                        {else}
                            <td><a href="view/{$tic.file_name}" class="user active">{$tic.file_name}</a>
                            <a href="#" class="delete" onclick="del_confirm('delete.php?ticket_id={$tic.ticket_id}&file_name={$tic.file_name}&tedit=1');">[削除]</a></td>
                        {/if}
					</tr>
            {/foreach}
			</table>
